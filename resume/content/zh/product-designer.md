---
title: "许嘉昭 – 全栈设计师"
variant: "product-designer"
label: "全栈设计师"
---

# 许嘉昭

**全栈设计师** <span class="resume-contact">邮箱: <a href="mailto:hello@xujiazhao.com">hello@xujiazhao.com</a> · 手机: 189-1016-3319 · 微信: xux-ai · 作品集: <a href="https://www.xux.ai">www.xux.ai</a></span>

---

## 核心能力

- **全栈产品构建者：**能够独立负责产品全生命周期——从构思、功能定义到 UX 设计和前端开发（React）——将像素级完美的产品从概念推进到上线
- **端到端设计与设计系统：**具备从用户研究、交互原型、视觉设计到设计系统搭建与维护的完整经验，支持产品高效迭代和设计表达一致性
- **AI 赋能设计：**熟练运用 AI 原生设计工作流——通过编程实现快速原型与独立落地，运用 Prompt Engineering 进行创意探索，并将大语言模型能力融入从概念到交付的完整产品体验
- **跨职能团队领导：**可领导由 PM、工程师及设计师组成的跨职能团队，推动设计在复杂项目中落地并提升整体价值
- **国际化视野：**长期在跨国团队设计国际化产品（北美、东南亚、Web3 等），能够在工作中平衡多元用户需求与文化差异

---

## 工作经历

### <img src="/assets/favicon/msft.svg" alt=""> 微软 - 高级产品设计师 / Copilot 播客产品负责人 <span class="resume-meta">北京, 2023.01 – 至今</span>

- **Copilot 播客产品：**从 0 到 1 发起并主导该项目，将播客能力引入微软 Copilot。管理由 1 名 PM、3 名工程师及外部贡献者组成的跨职能团队，全权负责产品路线图、功能优先级与迭代计划，直接向 Copilot CVP 汇报。
- 负责微软内容生态的全链路产品设计工作，覆盖 B 端到 C 端、从内容入驻到内容分发的完整闭环，通过设计驱动用户体验与平台营收的双重提升：
    - ** Copilot 内容体验设计：**主导微软 AI 产品 Copilot 中的内容设计，涵盖新闻、财经、体育等场景。探索 AI 如何提升内容的粘性与满意度，同时丰富 Copilot 的整体用户体验。
    - ** Bing & MSN 新闻产品设计：**负责 Bing News 与 MSN Feeds 的核心功能，包括必应热搜、重大新闻事件等；推出 AI 驱动的 News Digest（事件解读）等创新功能，显著提升用户体验与平台营收，并与团队共同推动产品在全球范围内超越竞争对手。
    - ** 合作伙伴生态B端平台：**为全球 3,000+ 中大型内容合作伙伴设计内容分发与管理平台，以及 AI 内容创作工具，提升合作方生产效率与触达率。内容供给覆盖微软全线 C 端产品（Windows、Copilot、Edge、MSN、Bing）。
    - ** 团队管理与设计效率：**搭建并维护跨产品的设计系统与交付规范（SOP），推动团队工具与流程优化；管理跨职能项目团队，协调 PM、工程师与设计师多方协作，全面提升团队产能与设计交付质量。
- 参与微软中国区设计团队 Studio 8 宣传工作，主导团队品牌建设、内部宣传与对外交流；包括北京大学课程合作，以及微软亚洲研发集团年会 KV 设计等。
- 推动 Vibe Coding 与各类 AI 工具在设计团队的落地，探索 AI + 设计 的新工作模式，提升设计效率。鼓励设计师承担更多产品经理职能，扩大设计在团队中的影响力。同时推动团队工具与流程优化，全面提升产能与团队幸福感。


### <img src="/assets/favicon/aqua-icon.webp" alt=""> AQUA Exchange – 产品、设计与开发 <span class="resume-meta">[兼职] 2025.12 – 至今</span>

- 一款基于 Hyperliquid 构建的 Web3 移动优先衍生品交易所，支持全品类永续合约交易、名人与 AI 跟单、Hyperliquid Vaults 金库及推荐返佣。已上线 iOS 和 Android。
- 独立负责产品与设计全流程——从产品构思、功能定义、UX 设计到前端开发（React），将像素级完美的交易应用从概念推进到上线。
- 端到端构建每一个页面：定义全部产品功能、设计完整用户体验、实现全部前端 - 展现产品+设计+工程的全栈能力。

### <img src="/assets/favicon/ant.svg" alt=""> 蚂蚁国际 - 体验设计师 <span class="resume-meta">上海, 2020.04 – 2022.12</span>

- 参与 Alipay+ 全球支付网络的设计工作，涵盖用户端（移动端与桌面端支付体验）以及机构端（商户入驻官网、帮助平台、开发集成流程、文档中心与开发者中心等）的全链路体验设计。
- 负责支付宝用户在苹果 App Store 的支付体验设计，包括支付方式签约流程及支付宝内「苹果专区」的体验设计。苹果专区为苹果用户提供会员权益发放、账单管理等功能，提升品牌专属感与用户粘性。
- 设计淘宝海外与天猫国际商城东南亚及港澳台地区的支付体验设，优化支付流程，提升支付便捷度与成功率。
- 主导蚂蚁集团首批 Figma 采购与落地计划，推动海外 SaaS 工具在合规、数据安全与采购流程上的完善，并承担集团层面 Figma 账号与使用规范的管理工作。

---

## 教育背景

<!-- HIDDEN
### <img src="/assets/favicon/thu.svg" alt=""> 清华大学经济管理学院 - 全球工商管理硕士 <span class="resume-meta">北京, 2024.09 – 至今</span>

- 班长，年级联络人。积极推动校企合作与资源整合，组织多场行业参访、校友座谈和企业闭门分享活动，广泛链接清华经管校友网络与企业导师资源；
- 任MBA具身智能俱乐部设计部部长，对接具身智能、机器人、AI初创等企业，组织参访、圆桌对话等。
HIDDEN -->

### <img src="/assets/favicon/accd.svg" alt=""> 艺术中心设计学院 - 产品设计专业理学学士 <span class="resume-meta">洛杉矶, 2016.09 – 2021.04</span>

- 辅修商学，荣誉毕业生；曾赴INSEAD欧洲工商管理学院交换学习；曾于小米、科勒工业设计团队实习。

---

## 技能、语言与其他

- **语言：** 中文（母语），英文（商务流利）

- **技能：** Figma、Sketch、Adobe Creative Suite、设计系统、原型设计、用户研究、可用性测试、精益交互设计、HTML/CSS、React、Vibecoding、AI设计、产品策略、增长设计、敏捷工作模式、工作坊策划

- **兴趣领域：**人工智能应用、演讲表达、社会研究